...
# urn
# samp <- sample_n(urn, size = 30, replace = TRUE)
# tib <- tibble(replicate = c(rep(1:5), 6), 
#       samp = samp)

# View(urn)

View(tibble(replicate = c(rep(1:5)), 
...
tib <- tibble(replicate = 1:1000) %>%
  ...
  mutate(num_red = map_int(samp, ~ sum(.x$color == "red"))) %>%
  mutate(red_percent = num_red/50 * 100) %>%
  summarize(prob = sum(red_percent > 50)/10)
